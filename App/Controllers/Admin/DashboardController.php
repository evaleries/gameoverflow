...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $incomes = Payment::raw('SELECT MONTHNAME(created_at) as month, SUM(amount) as income FROM `payments` WHERE status = :status AND YEAR(created_at) = YEAR(CURRENT_DATE) GROUP BY MONTH(created_at)', ['status' => Payment::CONFIRMED]);
        $sales = Order::raw('SELECT count(*) as sales FROM orders WHERE status = :status AND YEAR(created_at) = YEAR(CURRENT_DATE) GROUP BY MONTH(created_at)', ['status' => Order::COMPLETED]);
